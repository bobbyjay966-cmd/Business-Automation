...
import { createAutoSubscription, findOrCreateStripeCustomer, isStripeLive } from './stripe-billing';
import {
  isCallRailEnabled,
...
async function tryAutoSubscribe(
  ...
  const siteByTarget: Record<string, GeneratedSite | undefined> = {};
  for (const s of sites) siteByTarget[s.targetId] = s;

  const numbersMatching = (t: NicheCityTarget): TrackingNumber[] => {
  ...
  const billable = prospects.find(
    (p) =>
      p.targetId === targetReady.id
      && (p.pitchStatus === 'Pitched' || p.pitchStatus === 'Trial')
      && p.email
      && p.email.trim() !== ''
  ...
  log.push(
    `💳 STRIPE AUTO-SUB: "${billable.name}" is ready to lease "${liveSite?.domainName || targetReady.city + ' ' + targetReady.niche}". Creating subscription + invoice...`,
    'income',
  );
  ...
  try {
    ...
    billable.subscriptionStartDate = new Date().toISOString();
    ...
    log.push(
      `📈 MRR+$${(result.amountDue / 100).toFixed(2)}! "${billable.name}" is now an active RENTED tenant.`,
      'income',
    );
    return true;
  } catch (err: any) {
...
/**
 * Run one decision cycle. Safe to invoke from anywhere:
 *   - a `setInterval` started in `server.ts` on a long-lived process, or
 *   - a serverless function triggered by Vercel Cron / cron-job.org / etc.
 *
 * Returns a structured result so the client UI can render a faithful
 * "what just happened" log even though the work happened server-side.
 */
export async function runAutopilotCycle(): Promise<AutopilotCycleResult> {
  const start = Date.now();
  const log = makeLogBuffer();

  // Set running flag so the UI can show "Cycle in progress…" instead of
  // "Waiting for next server cycle…" during long LLM calls.
  isCycleRunning = true;

  // Hard timeout so a stuck LLM call never blows the function budget.
  let timedOut = false;
  ...
  try {
    ...
    if (!settings.isAutopilotOn) {
      log.push('🛑 Autopilot is OFF. Toggle "Start AI Autopilot" in the UI to begin.', 'info');
      return {
        ranAction: false,
        action: 'skipped_off',
    ...
    // 1. Add a new (niche × city) target market?
    ...
    // 2. Scrape a target (priority P1/P2/P3, dedup, 24h cooldown)?
    const scraped = await tryScrapeForTarget(log);
    if (scraped) {
      return finish(
        log, start, 'scrape_target',
        `Scraped ${scraped.newLeadCount} fresh leads for ${scraped.target.niche}/${scraped.target.city}.`,
      );
    }
    if (timedOut) return timeoutResult(log, start);

    // 2a. Create a Stripe customer for ONE verified lead (phone+email)?
    ...
    // 6. Send ONE trial email for a prospect that has a site + no trial yet?
    if (await trySendTrialEmail(log)) {
      return finish(log, start, 'send_trial_email', 'Sent one trial email.');
    }
    if (timedOut) return timeoutResult(log, start);

    // 7. Auto-subscribe ready prospects via Stripe ($450/mo plan)?
    ...
    // 8. Idle scan. Real revenue depends on async webhooks firing
    //    (CallRail inbounds, Stripe successful payments); this cycle
    //    has nothing synchronous to do.
    log.push('🧘 Portfolio audit complete. No actionable targets this tick.', 'info');
